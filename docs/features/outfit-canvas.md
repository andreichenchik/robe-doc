...
Let users visually combine clothing pieces into a look by dragging and positioning item cutouts on a canvas.
...
- Canvas state is synchronized with the backend.

> [!NOTE]
> **Undefined — requires clarification:**
> - Can items be scaled (pinch-to-zoom) and rotated on the canvas?
> - Is there a z-order control (bring to front / send to back)?
> - Can the user undo/redo canvas actions?
> - Is the canvas state auto-saved or explicitly saved by the user?
> - Can the canvas generate a static collage image (e.g. for sharing)?
> - Maximum number of items on a single canvas.
> - Canvas background — is it configurable or fixed?
